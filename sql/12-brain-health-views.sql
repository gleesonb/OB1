...
CREATE OR REPLACE VIEW public.ops_source_volume_24h WITH (security_invoker = true) AS
SELECT
...
CREATE OR REPLACE VIEW public.ops_recent_thoughts WITH (security_invoker = true) AS
SELECT
  id,
  created_at,
...
CREATE OR REPLACE VIEW public.ops_enrichment_gaps WITH (security_invoker = true) AS
SELECT
  id,
  created_at,
  coalesce(type, 'unknown') AS type,
...
CREATE OR REPLACE VIEW public.ops_type_distribution WITH (security_invoker = true) AS
SELECT
...
BEGIN
  IF to_regclass('public.ingestion_jobs') IS NOT NULL THEN
    EXECUTE $v$
      CREATE OR REPLACE VIEW public.ops_ingestion_summary WITH (security_invoker = true) AS
      SELECT
        status,
...
BEGIN
  IF to_regclass('public.entity_extraction_queue') IS NOT NULL THEN
    EXECUTE $v$
      CREATE OR REPLACE VIEW public.ops_stalled_entity_queue WITH (security_invoker = true) AS
      SELECT
        thought_id,
        status,
...
BEGIN
  IF to_regclass('public.entity_extraction_queue') IS NOT NULL THEN
    EXECUTE $v$
      CREATE OR REPLACE VIEW public.ops_graph_coverage WITH (security_invoker = true) AS
      SELECT
...
-- ============================================================
-- 10. LOCK DOWN API ROLES
--     Supabase default privileges grant anon/authenticated full access
--     to new objects in public, and the security advisor flags any
--     security-definer view there (0010_security_definer_view). Every
--     view above is created WITH (security_invoker = true); this block
--     removes the inherited grants so only service_role can read them.
-- ============================================================
